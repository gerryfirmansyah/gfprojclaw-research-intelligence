...
def main():
    ...
    with db() as conn, conn.cursor() as cur:
        ...
        profile_name=project_name='ALL'
        if project:
            cur.execute('SELECT rp.name AS profile_name, rj.name AS project_name FROM research_project rj JOIN research_profile rp ON rp.id=rj.profile_id WHERE rj.id=%s', (project,))
            meta=cur.fetchone() or {}; profile_name=meta.get('profile_name','UNKNOWN'); project_name=meta.get('project_name','UNKNOWN')

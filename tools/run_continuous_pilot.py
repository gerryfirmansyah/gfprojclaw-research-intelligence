...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--project-id'); ap.add_argument('--simulate-failure'); ap.add_argument('--dry-run',action='store_true'); ap.add_argument('--max-attempts',type=int,default=2); ap.add_argument('--retry-delay-seconds',type=float,default=5.0); ap.add_argument('--idempotency-key'); a=ap.parse_args()
    if a.max_attempts < 1: raise SystemExit('--max-attempts must be >= 1')
    if a.retry_delay_seconds < 0 or a.retry_delay_seconds > 300: raise SystemExit('--retry-delay-seconds must be between 0 and 300')
    ...
    for source in stages:
        ...
        if failed:
            ...
            if a.max_attempts > 1:
                if not a.dry_run and a.retry_delay_seconds: time.sleep(a.retry_delay_seconds)

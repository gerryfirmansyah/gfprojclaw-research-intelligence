...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--project-id',required=True); ap.add_argument('--hours',type=int,default=24); ap.add_argument('--send',action='store_true'); a=ap.parse_args()
    renderer=Path(__file__).with_name('render_telegram_daily_digest.py')
    msg=subprocess.check_output([sys.executable,str(renderer),'--project-id',a.project_id,'--hours',str(a.hours)],text=True).strip()
    if not a.send:

...
def build():
    ...
    for i,c in enumerate(context(),1):
        ...
        focus=c.get('research_intent') or c.get('profile_summary') or 'Fokus belum diisi'
        lines += ['',f"{label} — {c['profile_name']}",f"Project: {c['project_name']}",f"Fokus: {focus}",f"Perubahan: {len(x['new_papers'])} paper baru; {len(x['new_claims'])} Claim baru; {len(x['new_contradictory_evidence'])} bukti menantang/kontradiktif; {len(x['advice_critic_changes'])} perubahan Advice & Critic."]
    ...
    for label,c,x in states:
        ...
        lines.append(f"{label} — {c['profile_name']}: {nr} NEEDS_REVIEW; {co} CONTESTED.")
        cov=x.get('coverage') or {}; cs=cov.get('counter_search_state'); lines.append(f"  Counter-search: {cs if cs else 'NOT_AVAILABLE'}.")

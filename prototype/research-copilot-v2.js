...
function renderDashboard() {
  ...
  if(sf){
    ...
    const paperPanel=document.getElementById("paper-list")?.closest(".panel");const paperHeading=paperPanel?.querySelector(".panel-header h2");if(paperHeading)paperHeading.textContent="Evidence Aktif";document.getElementById("paper-list").innerHTML=sessionEvidenceRows();
...
async function loadTodayMetrics() {
  ...
  try {
    ...
    const handoffRaw=sessionStorage.getItem("gfprojclaw-explorer-handoff");
    if(handoffRaw){try{const hh=JSON.parse(handoffRaw),ff=hh?.selected_gap_opportunity?.focused_investigation;if(ff){const incoming=(ff.assessments||[]).length;setMetric("metric-papers",incoming,`Focused Investigation: ${incoming} evidence dibawa · NON-CANONICAL · Project Corpus tetap ${papers.length} persisted Works`,"papers");setMetric("metric-decisions",1,`HUMAN direction: ${ff.human_direction||"NOT_RECORDED"} · NON-CANONICAL handoff`,"decisions");setMetric("metric-changes",0,"Belum ada canonical ChangeEvent dari Focused Investigation ini","changes");}}catch(_){}}
...
function renderJourney() {
  const h=activeSession(),f=activeFocused();
  if(f){
    ...
    const weak=[];
    if(rows.length && reviewed<rows.length) weak.push(`Assessment HUMAN baru ${reviewed}/${rows.length} paper`);
    weak.push("Evidence Qualification W5 perlu diperkuat");
    weak.push("Counter-search / falsification W10 perlu diperkuat");
    const nextDecision=f.human_direction==="CONTINUE_INVESTIGATION" ? "Verifikasi boundary/counter-evidence sebelum membentuk candidate Research Problem / RQ." : "Konfirmasi arah HUMAN sebelum reasoning dilanjutkan.";
    const controlTower=`<section class="journey-control-tower"><div class="journey-control-head"><div><span>RESEARCH CONTROL TOWER</span><h2>Helicopter view sesi riset aktif</h2><p>Ringkasan ini hanya membaca Research Reasoning Package aktif; tidak mencampur project atau sesi lama.</p></div><div class="journey-position">POSISI SEKARANG<strong>C1–C4 · VERIFY / REASON</strong></div></div><div class="journey-summary-grid"><article><span>Objek aktif</span><strong>${escapeHtml(f.io_id)} · ${escapeHtml(f.io_title)}</strong><p>Investigation Opportunity pilihan HUMAN; belum validated research gap.</p></article><article><span>Evidence state</span><strong>${rows.length} paper · ${reviewed} direview HUMAN</strong><p>${f.supports||0} mendukung · ${f.challenges||0} menantang · ${f.unclear||0} belum jelas.</p></article><article><span>Arah HUMAN</span><strong>${escapeHtml(f.human_direction||"NOT_RECORDED")}</strong><p>Handoff Workspace → Copilot tercatat untuk sesi aktif.</p></article><article class="attention"><span>Yang perlu diperkuat</span><strong>${escapeHtml(weak.join(" · "))}</strong><p>Ini diagnosis workflow, bukan verdict ilmiah.</p></article></div><div class="journey-next-decision"><div><span>NEXT HUMAN CHECKPOINT</span><strong>${escapeHtml(nextDecision)}</strong></div><div><span>LOOP-BACK BILA PERLU</span><strong>C4 → W5/W10 · C5 → W4/W5</strong></div></div></section>`;
    return sessionHeader("Perjalanan Riset","Peta end-to-end scientific reasoning: Workspace membangun evidence; Copilot v2 mengubah evidence pilihan HUMAN menjadi reasoning, RQ, design, dan readiness.")+controlTower+`<div class="journey-role-banner"><span>ANDA SEDANG BERADA DI RESEARCH COPILOT v2</span><strong>Reason → Verify → Formulate → Design → Challenge → Decide → Evolve</strong><p>Jika tugas Anda masih memetakan semesta, screening, memilih paper, atau memilih IO, kembali ke Research Workspace.</p></div><div class="journey-v2-boundary"><div><span>RESEARCH WORKSPACE · DISCOVERY + EVIDENCE CONSTRUCTION</span><h2>Explore → Screen → Qualify → Analyze → Synthesize → Investigate</h2><p>Menjawab: “apa yang layak saya investigasi dan mengapa?” HUMAN memilih evidence, IO, assessment, dan arah.</p></div><div class="journey-v2-arrow">Research Reasoning Package →</div><div><span>RESEARCH COPILOT v2 · SCIENTIFIC REASONING + PROJECT FORMATION</span><h2>Reason → Verify → Formulate → Design → Challenge → Evolve</h2><p>Menjawab: “bagaimana investigasi ini menjadi problem, defensible RQ, theory positioning, method, design, dan contribution?”</p></div></div><div class="journey-v2-grid"><section class="card"><h3>1 · Research Workspace · W1–W13</h3>${cards(ws,'workspace')}</section><section class="card"><h3>2 · Research Copilot v2 · C1–C16</h3>${cards(cp,'copilot')}</section></div><section class="card journey-loops"><h3>Research is iterative · loop yang disengaja</h3><p>Flow bukan waterfall. Evidence baru atau contradiction dapat membawa HUMAN kembali ke tahap sebelumnya.</p>${loopCards}</section><div class="callout warning"><strong>Batas keputusan:</strong> ${escapeHtml(f.io_id)} tetap Investigation Opportunity. Validated gap, novelty, Research Question, contribution, dan canonical knowledge change hanya muncul setelah verification tambahan dan keputusan eksplisit HUMAN.</div>`;
  }
...
function sessionHeader(title,subtitle){const f=activeFocused();return commonHeader(title,subtitle,"real")+`<div class="active-session-strip"><strong>RESEARCH COPILOT v2 · ACTIVE REASONING SESSION · ${escapeHtml(f?.io_id||"IO")}: ${escapeHtml(f?.io_title||"")}</strong><br>${escapeHtml(f?.question||"")}<br><small>NON-CANONICAL sampai HUMAN membuat explicit scientific write/decision.</small></div>`}
function evidenceLinks(a){const oa=a.openalex_url||(a.openalex_id?(String(a.openalex_id).startsWith("http")?a.openalex_id:`https://openalex.org/${a.openalex_id}`):"");const doi=a.doi_url||(a.doi?(String(a.doi).startsWith("http")?a.doi:`https://doi.org/${String(a.doi).replace(/^doi:/i,"")}`):"");return `<span class="evidence-links">${oa?`<a href="${escapeHtml(oa)}" target="_blank" rel="noopener">OpenAlex ↗</a>`:`<span>OpenAlex tidak tersedia</span>`}${doi?`<a href="${escapeHtml(doi)}" target="_blank" rel="noopener">DOI ↗</a>`:`<span>DOI tidak tersedia</span>`}</span>`}
function sessionEvidenceRows(filter=null){const f=activeFocused(),rows=(f?.assessments||[]).filter(filter||(()=>true));return `<div class="table-frame active-session-table"><table><thead><tr><th>Paper</th><th>Source</th><th>Surfaced as</th><th>Assessment HUMAN</th><th>Catatan HUMAN</th></tr></thead><tbody>${rows.map(a=>`<tr><td><strong>${escapeHtml(a.paper||"Paper")}</strong></td><td>${evidenceLinks(a)}</td><td>${escapeHtml(a.surfaced_as||"—")}</td><td>${escapeHtml(a.status||"NOT_REVIEWED")}</td><td>${escapeHtml(a.note||"—")}</td></tr>`).join("")||`<tr><td colspan="5">Tidak ada evidence pada state ini.</td></tr>`}</tbody></table></div>`}
function renderActiveOpportunities(){const f=activeFocused(),rows=f.assessments||[],surfSupport=rows.filter(a=>String(a.surfaced_as||"").includes("SUPPORT")).length,surfChallenge=rows.filter(a=>String(a.surfaced_as||"").includes("CHALLENGE")).length,hSupport=rows.filter(a=>a.status==="SUPPORTS").length,hChallenge=rows.filter(a=>a.status==="CHALLENGES").length,hUnclear=rows.filter(a=>a.status==="UNCLEAR").length,reclassified=rows.filter(a=>String(a.surfaced_as||"").includes("CHALLENGE")&&a.status==="SUPPORTS"),trueChallenge=rows.filter(a=>a.status==="CHALLENGES"),unclear=rows.filter(a=>a.status==="UNCLEAR"||a.status==="NOT_REVIEWED");const evidenceDetails=`<details class="io-evidence-details"><summary>Evidence yang mendasari (${rows.length}) · buka provenance paper</summary>${sessionEvidenceRows()}</details>`;return sessionHeader("Peluang Riset","Reasoning workspace untuk memahami mengapa Investigation Opportunity ini layak diperiksa lebih lanjut.")+`<section class="io-impact"><div class="io-question"><span>INVESTIGATION OPPORTUNITY DIPILIH HUMAN</span><h2>${escapeHtml(f.io_id)} · ${escapeHtml(f.io_title)}</h2><p>${escapeHtml(f.question)}</p></div><div class="io-grid"><article><span>OBSERVATION</span><strong>${hSupport}/${rows.length} evidence dinilai HUMAN mendukung arah pemeriksaan ini.</strong><p>Ini menunjukkan arah investigasi mempunyai dukungan pada evidence set terpilih, tetapi belum membuktikan generalisasi.</p></article><article><span>COMPARE SIGNAL</span><strong>Mesin: ${surfSupport} support signal · ${surfChallenge} challenge candidate.</strong><p>Sesudah review HUMAN: ${hSupport} mendukung · ${hChallenge} menantang · ${hUnclear} belum jelas.</p></article><article><span>RECLASSIFICATION</span><strong>${reclassified.length} challenge candidate → SUPPORTS oleh HUMAN.</strong><p>${reclassified.length?"Candidate yang awalnya disurface untuk menguji arah ternyata tidak menjadi counter-evidence setelah review HUMAN.":"Belum ada perubahan klasifikasi surfaced-as oleh assessment HUMAN."}</p></article><article><span>COUNTER-EVIDENCE</span><strong>${trueChallenge.length} evidence dinilai HUMAN benar-benar menantang.</strong><p>${trueChallenge.length?"Perbedaan ini perlu dibaca terhadap context, method, unit, mechanism, dan outcome.":"Belum ada counter-evidence pada evidence set ini; itu bukan bukti bahwa counter-evidence tidak ada."}</p></article></div><div class="io-interpretation"><h3>Apa arti pola ini?</h3><p>${hSupport===rows.length?"Evidence set yang direview bergerak searah terhadap pertanyaan investigasi.":`Mayoritas evidence yang direview (${hSupport}/${rows.length}) bergerak searah, tetapi terdapat ${hChallenge} challenge dan ${hUnclear} yang belum jelas.`} ${reclassified.length?`${reclassified.length} paper yang semula disurface sebagai challenge candidate justru dinilai HUMAN mendukung; karena itu label surfaced-as tidak boleh diperlakukan sebagai verdict ilmiah.`:""}</p><p><strong>Uncertainty:</strong> pola ini hanya berasal dari evidence set terpilih. Belum diketahui apakah ia bertahan pada corpus lebih luas, full text, context, method, unit, dan outcome yang berbeda.</p></div><div class="io-next"><h3>Yang perlu diverifikasi berikutnya</h3><div class="io-next-grid"><div><strong>1 · Cari counter-evidence nyata</strong><p>Periksa paper/context lain yang menghasilkan mechanism → outcome berbeda.</p></div><div><strong>2 · Uji context sensitivity</strong><p>Bandingkan apakah pola berubah menurut lokasi, governance setting, unit, atau population.</p></div><div><strong>3 · Uji method dependence</strong><p>Periksa apakah finding berubah karena design, sample, atau analytical method.</p></div><div><strong>4 · Periksa batas evidence</strong><p>Verifikasi full text dan limitation sebelum HUMAN menyatakan gap atau novelty.</p></div></div></div><div class="io-boundary"><strong>Research implication saat ini</strong><p>IO ini layak dipertahankan sebagai <em>investigation direction</em>. Sistem belum memiliki dasar untuk menyatakannya sebagai validated research gap, novelty claim, atau kebenaran umum.</p></div>${evidenceDetails}</section>`}
...
function renderReasoningWorkbench(){
 const f=activeFocused(),rows=f?.assessments||[],reviewed=rows.filter(x=>x.status&&x.status!=="NOT_REVIEWED"),support=rows.filter(x=>x.status==="SUPPORTS").length,challenge=rows.filter(x=>x.status==="CHALLENGES").length,unclear=rows.filter(x=>x.status==="UNCLEAR"||x.status==="NOT_REVIEWED").length;
 const contradiction=challenge?`${challenge} paper dinilai HUMAN menantang arah awal.`:unclear?`${unclear} paper masih unclear/not reviewed.`:"Belum ada counter-evidence pada evidence set terpilih; counter-search tetap diperlukan.";
 const candidate=`Dalam kondisi apa mekanisme yang ditelusuri pada ${f?.io_id||"IO aktif"} menghasilkan outcome berbeda, dan apakah perbedaan tersebut dijelaskan oleh context, method, unit, atau boundary evidence?`;
 return sessionHeader("Reasoning Workbench","C1–C7 · Dari Investigation Opportunity menuju candidate Research Problem dan Research Question tanpa melompati verification.")+`<div class="journey-role-banner"><span>RESEARCH COPILOT v2 · SCIENTIFIC REASONING</span><strong>C1 Brief → C2 Reason → C3 Verify → C4 Falsify → C5 HUMAN Decide → C6 Candidate RQ → C7 HUMAN RQ Decision</strong><p>Semua keluaran di bawah adalah reasoning aid. Tidak ada gap, novelty, RQ, atau project yang menjadi canonical tanpa keputusan HUMAN.</p></div><div class="reasoning-stage-grid"><article class="card"><span class="eyebrow">C1 · SESSION BRIEF</span><h3>${escapeHtml(f?.io_id||"IO")} · ${escapeHtml(f?.io_title||"Investigation Opportunity")}</h3><p>${escapeHtml(f?.question||"Question belum tercatat")}</p><small>${rows.length} evidence · HUMAN: ${support} supports · ${challenge} challenges · ${unclear} unresolved</small></article><article class="card"><span class="eyebrow">C2 · INVESTIGATION REASONING</span><h3>Working interpretation</h3><p>Evidence set memberi alasan untuk mempertahankan arah investigasi, tetapi generalisasi belum dapat dibuat. Perbedaan context, method, unit, mechanism, dan outcome harus tetap menjadi explanatory alternatives.</p></article><article class="card"><span class="eyebrow">C3 · EVIDENCE VERIFICATION</span><h3>Verification state</h3><p>${reviewed.length}/${rows.length} paper memiliki assessment HUMAN. Provenance paper tetap dapat ditelusuri di Penjelajah Bukti.</p><button class="secondary-action" type="button" data-reasoning-open="evidence">Buka evidence provenance →</button></article><article class="card"><span class="eyebrow">C4 · FALSIFICATION</span><h3>Apa yang dapat melemahkan interpretasi?</h3><p>${escapeHtml(contradiction)}</p><p>Counter-search perlu mencari context/outcome/method yang menghasilkan pola berbeda, bukan hanya menambah supporting papers.</p></article></div><section class="card reasoning-decision"><span class="eyebrow">C5 · HUMAN DECISION CHECKPOINT</span><h3>Apakah evidence cukup untuk merumuskan candidate Research Question?</h3><div class="reasoning-choice-row"><button type="button" data-rq-decision="CONTINUE">Cukup untuk candidate RQ</button><button type="button" data-rq-decision="REVISE">Revisi arah investigasi</button><button type="button" data-rq-decision="INSUFFICIENT">Evidence belum cukup</button></div><div id="rq-decision-state" class="callout">Belum ada keputusan HUMAN pada checkpoint ini.</div></section><section class="card"><span class="eyebrow">C6 · RESEARCH PROBLEM + CANDIDATE RQ</span><h3>Draft untuk dikritik HUMAN</h3><p><strong>Problem frame:</strong> evidence awal belum menjelaskan apakah mechanism → outcome bertahan lintas context/method/unit dan di mana boundary-nya.</p><label><strong>Candidate Research Question</strong><textarea id="candidate-rq" rows="3">${escapeHtml(candidate)}</textarea></label><small>Candidate ini dibentuk dari IO + assessment HUMAN. Ia bukan RQ final.</small></section><section class="card reasoning-decision"><span class="eyebrow">C7 · HUMAN RQ DECISION</span><h3>HUMAN menentukan nasib candidate RQ</h3><div class="reasoning-choice-row"><button type="button" data-final-rq="ACCEPT">Terima sebagai working RQ</button><button type="button" data-final-rq="REVISE">Revisi sendiri</button><button type="button" data-final-rq="REJECT">Tolak / kembali reasoning</button></div><div id="final-rq-state" class="callout">Belum ada working RQ yang diputuskan HUMAN.</div></section><section class="card"><span class="eyebrow">C8–C15 · RESEARCH FORMATION GATE</span><h3>Setelah working RQ diterima HUMAN</h3><p><strong>C8</strong> Construct clarification → <strong>C9</strong> Theory positioning → <strong>C10</strong> Method intelligence → <strong>C11</strong> Research design → <strong>C12</strong> Potential contribution → <strong>C13</strong> Novelty challenge → <strong>C14</strong> Argument/evidence audit → <strong>C15</strong> Research readiness.</p><div class="callout warning">Tahap ini tidak boleh dilewati langsung dari IO. Working RQ harus melewati C5–C7; novelty harus diuji dengan counter/prior-solution search; readiness tetap keputusan HUMAN.</div></section>`;
}
function bindReasoningWorkbench(){
 document.querySelectorAll("[data-reasoning-open]").forEach(b=>b.onclick=()=>showView(b.dataset.reasoningOpen));
 document.querySelectorAll("[data-rq-decision]").forEach(b=>b.onclick=()=>{const s=document.getElementById("rq-decision-state"),d=b.dataset.rqDecision;sessionStorage.setItem("gfprojclaw-rq-checkpoint",JSON.stringify({decision:d,scientific_decision:true,canonical_write:false}));s.innerHTML=d==="CONTINUE"?"<strong>HUMAN: lanjut ke candidate RQ.</strong> Candidate tetap non-canonical sampai C7.":d==="REVISE"?"<strong>HUMAN: revisi arah.</strong> Kembali ke Investigation Reasoning / Workspace IO bila perlu.":"<strong>HUMAN: evidence belum cukup.</strong> Kembali ke Evidence Selection/Qualification atau lakukan counter-search.";});
 document.querySelectorAll("[data-final-rq]").forEach(b=>b.onclick=()=>{const q=document.getElementById("candidate-rq")?.value.trim(),d=b.dataset.finalRq,s=document.getElementById("final-rq-state");sessionStorage.setItem("gfprojclaw-working-rq",JSON.stringify({decision:d,research_question:q,scientific_decision:true,canonical_write:false}));s.innerHTML=d==="ACCEPT"?`<strong>Working RQ diterima HUMAN.</strong><p>${escapeHtml(q)}</p><small>Working RQ · NON-CANONICAL. Tahap berikutnya: construct → theory → method → design.</small>`:d==="REVISE"?"<strong>HUMAN memilih revisi.</strong> Edit candidate RQ lalu putuskan kembali.":"<strong>HUMAN menolak candidate RQ.</strong> Kembali ke C2/C4/C6; jangan lanjut ke theory/method.";});
}
const activeRenderers={journey:renderJourney,reasoning:renderReasoningWorkbench,opportunities:renderActiveOpportunities,evidence:renderActiveEvidence,evolution:renderActiveEvolution,review:renderActiveReview,coverage:renderActiveCoverage,profiles:renderActiveProfiles,telegram:renderActiveTelegram};
...
function showView(name) {
  ...
  if (sessionMode && name === "reasoning") bindReasoningWorkbench();
  if (!sessionMode) {
...
function renderExplorerHandoff(){
 ...
 document.getElementById("handoff-human").textContent=(h.selected_gap_opportunity?`Investigation Opportunity dipilih HUMAN: ${h.selected_gap_opportunity.focused_investigation?.io_title||h.selected_gap_opportunity.term}. `:"")+(h.human_learning_note||"Focused assessment dibawa dari Research Workspace bila tersedia.");
 const gap=document.getElementById("handoff-gap"),verify=document.getElementById("handoff-verification"),start=document.getElementById("handoff-start"),o=h.selected_gap_opportunity,focused=o?.focused_investigation;
 ...
 if(focused){
  gap.hidden=false;gap.innerHTML=`<strong>${escapeHtml(focused.io_id)} · ${escapeHtml(focused.io_title)}</strong><br><strong>Pertanyaan investigasi:</strong> ${escapeHtml(focused.question)}<br><small>Dipilih HUMAN setelah Compare & Contrast dan Cross-dimensional Analysis; belum merupakan validated research gap.</small>`;
  verify.hidden=false;verify.innerHTML=`<div class="advice-card"><strong>HUMAN Evidence Balance</strong><p>${focused.supports} mendukung · ${focused.challenges} menantang · ${focused.unclear} belum jelas · ${focused.not_reviewed} belum direview.</p></div><div class="advice-card"><strong>Arah HUMAN</strong><p>${escapeHtml(focused.human_direction||"CONTINUE_INVESTIGATION")}</p></div><div class="advice-card"><strong>Yang Copilot terima</strong><p>IO + pertanyaan + assessment per paper + catatan HUMAN + provenance surfaced-as.</p></div><div class="advice-card"><strong>Batas</strong><p>Assessment bukan probability of truth dan tidak menetapkan gap/novelty.</p></div>`;
  const ev=document.getElementById("handoff-evidence");ev.hidden=false;ev.classList.add("handoff-table-frame");ev.innerHTML=`<strong>Focused Investigation · ${focused.assessments?.length||0} evidence hasil IO</strong><p class="handoff-table-hint">Scroll kanan–kiri di dalam frame. Paper title tetap dalam bahasa asli.</p><div class="copilot-evidence-scroll"><table class="copilot-evidence-table"><thead><tr><th>Paper</th><th>Surfaced as</th><th>Assessment HUMAN</th><th>Catatan HUMAN</th></tr></thead><tbody>${(focused.assessments||[]).map(a=>`<tr><td><strong>${escapeHtml(a.paper||"Paper")}</strong></td><td>${escapeHtml(a.surfaced_as||"—")}</td><td><b>${escapeHtml(a.status||"NOT_REVIEWED")}</b></td><td>${escapeHtml(a.note||"—")}</td></tr>`).join("")}</tbody></table></div>`;
  start.textContent="Mulai reasoning Copilot dari Focused Investigation →";
  start.onclick=()=>{const next=document.getElementById("handoff-next-question");next.hidden=false;const unresolved=(focused.assessments||[]).filter(a=>a.status==="CHALLENGES"||a.status==="UNCLEAR"||a.status==="NOT_REVIEWED");const seed=focused.challenges?"Apa yang menjelaskan evidence yang dinilai HUMAN menantang—context, method, unit, mechanism, atau outcome?":unresolved.length?"Evidence apa yang perlu diverifikasi agar uncertainty pada Focused Investigation dapat dikurangi?":"Counter-evidence apa yang perlu dicari sebelum pola awal ini diperlakukan sebagai arah riset yang lebih kuat?";next.innerHTML=`<strong>Research Copilot · Next Verification Question</strong><p>Copilot melanjutkan dari keputusan HUMAN; assessment tidak diulang.</p><textarea id="human-next-question" rows="3"></textarea><button id="save-next-question" class="primary-action" type="button">Gunakan pertanyaan ini sebagai arah investigasi →</button><small> HUMAN dapat mengubah pertanyaan sepenuhnya. Ini bukan RQ final atau klaim gap.</small>`;document.getElementById("human-next-question").value=seed;document.getElementById("save-next-question").onclick=()=>{const q=document.getElementById("human-next-question").value.trim();sessionStorage.setItem("gfprojclaw-human-next-investigation",JSON.stringify({opportunity:focused.io_id,question:q,based_on_focused_human_assessment:true,scientific_decision:false,canonical_write:false}));next.insertAdjacentHTML("beforeend",`<div class="io-choice"><strong>Arah investigasi tersimpan di sesi.</strong><br>${escapeHtml(q)}<br><small>Langkah berikutnya: Copilot membantu menguji pertanyaan ini terhadap unresolved/challenge evidence sebelum retrieval diperluas.</small></div>`);renderHumanReasoningTrace();};start.disabled=true;start.textContent="REASONING COPILOT AKTIF · BERBASIS FOCUSED INVESTIGATION";next.scrollIntoView({behavior:"smooth",block:"start"});};
 }
 else if(o?.supporting_papers?.length){const ev=document.getElementById("handoff-evidence");ev.hidden=false;ev.innerHTML=`<strong>Evidence dibawa dari Explorer · ${escapeHtml(o.evidence_level||"ABSTRACT_ONLY")}</strong>${o.supporting_papers.map(p=>`<div class="advanced-paper"><strong>${escapeHtml(p.title||"JUDUL TIDAK TERSEDIA")}</strong><br><small>${p.source_url?`<a href="${escapeHtml(p.source_url)}" target="_blank" rel="noopener">OpenAlex ↗</a>`:""}${p.doi?` · <a href="${escapeHtml(p.doi)}" target="_blank" rel="noopener">DOI ↗</a>`:""}</small><details><summary>Baca abstract evidence</summary><p>${escapeHtml(p.abstract||"Abstract tidak tersedia")}</p></details></div>`).join("")}${o.counter_evidence_candidates?.length?`<hr><strong>Counter-evidence candidates · HUMAN challenge (${o.counter_evidence_candidates.length})</strong><p>Paper berikut belum otomatis menantang opportunity. Baca dan nilai apakah mekanisme/hasilnya benar-benar berlawanan.</p>${o.counter_evidence_candidates.map(p=>`<div class="advanced-paper counter-candidate"><strong>${escapeHtml(p.title||"JUDUL TIDAK TERSEDIA")}</strong><br><small>${p.source_url?`<a href="${escapeHtml(p.source_url)}" target="_blank" rel="noopener">OpenAlex ↗</a>`:""}${p.doi?` · <a href="${escapeHtml(p.doi)}" target="_blank" rel="noopener">DOI ↗</a>`:""}</small><details><summary>Baca abstract challenge</summary><p>${escapeHtml(p.abstract||"Abstract tidak tersedia")}</p></details></div>`).join("")}`:""}<p><strong>Counter-evidence protocol:</strong> ${escapeHtml(o.counter_evidence_instruction||"Cari bukti yang menantang opportunity ini.")}</p><small>Evidence ini belum canonical dan tidak menetapkan research gap.</small>`;}
 ...
 start.onclick=()=>{if(!o)return;start.textContent="VERIFIKASI DIMULAI · HUMAN REVIEW";start.disabled=true;document.getElementById("handoff-human").textContent=`Opportunity “${o.term}” dipilih untuk diverifikasi. Periksa supporting evidence, counter-evidence, abstrak/metode/konteks sebelum keputusan research gap. Scientific decision tetap milik HUMAN.`;const matrix=document.getElementById("handoff-evidence-matrix"),verdict=document.getElementById("handoff-human-verdict"),papers=[...(o.supporting_papers||[]).map(p=>({...p,discovery_role:"SUPPORTING_SIGNAL"})),...(o.counter_evidence_candidates||[]).map(p=>({...p,discovery_role:"CHALLENGE_CANDIDATE"}))];matrix.hidden=false;matrix.innerHTML=`<strong>Evidence Review Matrix · HUMAN</strong><p>Tandai hasil pembacaan abstract. Status ini adalah catatan review HUMAN, bukan klasifikasi otomatis.</p>${papers.map((p,i)=>`<div class="evidence-review-row" data-i="${i}"><strong>${escapeHtml(p.title||"JUDUL TIDAK TERSEDIA")}</strong><br><small>Masuk matrix sebagai: ${p.discovery_role==="CHALLENGE_CANDIDATE"?"challenge candidate":"supporting signal"} · ini bukan hasil review.</small><div class="evidence-review-actions"><button type="button" data-status="SUPPORTS">Mendukung</button><button type="button" data-status="CHALLENGES">Menantang</button><button type="button" data-status="UNCLEAR">Belum jelas</button></div><div class="evidence-review-dimensions"><input data-dim="context" placeholder="Konteks/negara/kota"><input data-dim="method" placeholder="Metode/desain"><input data-dim="unit" placeholder="Unit analisis"><input data-dim="outcome" placeholder="Outcome/temuan"><input data-dim="mechanism" placeholder="Mekanisme"></div><textarea rows="2" placeholder="Catatan HUMAN: limitation atau alasan penilaian…"></textarea></div>`).join("")}<button id="summarize-human-review" class="primary-action" type="button">Ringkas review HUMAN</button>`;document.getElementById("summarize-human-review").onclick=()=>{const reviews=[...matrix.querySelectorAll(".evidence-review-row")].map((row,i)=>({openalex_id:papers[i]?.openalex_id,title:papers[i]?.title,discovery_role:papers[i]?.discovery_role,status:row.dataset.status||"NOT_REVIEWED",dimensions:Object.fromEntries([...row.querySelectorAll("[data-dim]")].map(x=>[x.dataset.dim,x.value.trim()])),note:row.querySelector("textarea").value.trim()}));const n=x=>reviews.filter(r=>r.status===x).length;verdict.hidden=false;verdict.innerHTML=`<strong>HUMAN Evidence Balance</strong><br>Mendukung: ${n("SUPPORTS")} · Menantang: ${n("CHALLENGES")} · Belum jelas: ${n("UNCLEAR")} · Belum direview: ${n("NOT_REVIEWED")}<p><strong>Bandingkan alasan perbedaan:</strong> gunakan konteks, metode, unit analisis, outcome, dan mekanisme yang Anda isi pada setiap paper—bukan jumlah vote saja.</p><p><strong>Keputusan tetap terbuka.</strong> Evidence balance bukan truth probability dan belum membuktikan research gap.</p><button id="save-human-review" class="secondary-action" type="button">Simpan review sementara di sesi</button> <button id="derive-next-question" class="primary-action" type="button">Susun pertanyaan verifikasi berikutnya</button>`;document.getElementById("save-human-review").onclick=()=>{sessionStorage.setItem("gfprojclaw-human-evidence-review",JSON.stringify({opportunity:o.id||o.term,reviews,scientific_decision:false,canonical_write:false}));document.getElementById("save-human-review").textContent="TERSIMPAN DI SESI · NON-CANONICAL";renderHumanReasoningTrace();};document.getElementById("derive-next-question").onclick=()=>{const next=document.getElementById("handoff-next-question"),support=n("SUPPORTS"),challenge=n("CHALLENGES"),unclear=n("UNCLEAR")+n("NOT_REVIEWED");next.hidden=false;const filled=reviews.filter(r=>Object.values(r.dimensions||{}).some(Boolean)),contrast=filled.filter(r=>r.status==="CHALLENGES"||r.status==="UNCLEAR").slice(0,3),dimSummary=contrast.map(r=>`${r.title}: ${Object.entries(r.dimensions||{}).filter(([,v])=>v).map(([k,v])=>`${k}=${v}`).join("; ")}`).filter(x=>!x.endsWith(": "));let focus=challenge?"Jelaskan mengapa evidence yang menantang berbeda: konteks, metode, unit analisis, atau mekanisme?":unclear?"Evidence masih belum cukup: paper/metode/konteks apa yang harus diperiksa agar opportunity dapat diuji lebih kuat?":"Supporting evidence mendominasi review awal: cari secara sengaja studi dengan hasil, mekanisme, atau konteks yang berlawanan sebelum menyimpulkan gap.";next.innerHTML=`<strong>Next Verification Question · HUMAN decides</strong><p>${escapeHtml(focus)}</p><p><strong>Evidence balance saat ini:</strong> ${support} mendukung · ${challenge} menantang · ${unclear} belum pasti.</p>${dimSummary.length?`<div class="callout"><strong>Perbedaan yang dicatat HUMAN</strong><br>${dimSummary.map(escapeHtml).join("<br>")}</div>`:"<p><small>Isi dimensi evidence agar pertanyaan berikutnya dapat berpijak pada perbedaan yang Anda temukan.</small></p>"}<textarea id="human-next-question" rows="3" placeholder="HUMAN dapat mengubah atau mengganti pertanyaan ini sepenuhnya."></textarea><button id="save-next-question" class="secondary-action" type="button">Simpan sebagai arah investigasi sementara</button><small> Sistem hanya menyusun prompt dari review HUMAN; ini bukan research question final, gap, atau keputusan ilmiah.</small>`;document.getElementById("human-next-question").value=dimSummary.length?`${focus} Periksa khusus perbedaan yang dicatat HUMAN: ${dimSummary.join(" | ")}`:focus;document.getElementById("save-next-question").onclick=()=>{const q=document.getElementById("human-next-question").value.trim();const draft={opportunity:o.id||o.term,question:q,based_on_human_review:true,scientific_decision:false,canonical_write:false};sessionStorage.setItem("gfprojclaw-human-next-investigation",JSON.stringify(draft));document.getElementById("save-next-question").textContent="TERSIMPAN · HUMAN INVESTIGATION DRAFT";renderHumanReasoningTrace();const plan=document.getElementById("handoff-investigation-plan");plan.hidden=false;plan.innerHTML=`<strong>Investigation Plan · DRAFT untuk HUMAN</strong><p><strong>Pertanyaan:</strong> ${escapeHtml(q)}</p>${dimSummary.length?`<div class="callout"><strong>Evidence contrast yang memicu investigasi</strong><br>${dimSummary.map(escapeHtml).join("<br>")}</div>`:""}<ol><li>Periksa paper berstatus CHALLENGES/UNCLEAR terlebih dahulu.</li><li>Bandingkan konteks, unit analisis, metode, mekanisme, dan outcome dengan supporting papers.</li><li>Perluas pencarian hanya bila evidence pada subset belum cukup untuk menjawab pertanyaan.</li><li>Catat evidence yang mendukung dan menantang secara terpisah.</li><li>Kembali ke HUMAN untuk menentukan apakah opportunity ditolak, direvisi, diteruskan, atau memerlukan pencarian baru.</li></ol><label><strong>Kriteria cukup menurut HUMAN</strong><textarea id="human-sufficiency" rows="2" placeholder="Contoh: cukup setelah metode dan hasil dari paper yang menantang sudah diperiksa."></textarea></label><button id="save-investigation-plan" class="primary-action" type="button">Simpan Investigation Plan sementara</button><small> Draft ini tidak mengubah scope/RQ/gap secara otomatis dan tidak menulis ke canonical project state.</small>`;document.getElementById("save-investigation-plan").onclick=()=>{const suff=document.getElementById("human-sufficiency").value.trim();const ip={...draft,evidence_contrasts:dimSummary,sufficiency_criterion:suff,steps:["review challenging/unclear evidence","compare context/unit/method/mechanism/outcome","expand retrieval only if needed","separate supporting/challenging evidence","return to HUMAN decision"],scientific_decision:false,canonical_write:false};sessionStorage.setItem("gfprojclaw-human-investigation-plan",JSON.stringify(ip));document.getElementById("save-investigation-plan").textContent="TERSIMPAN · NON-CANONICAL PLAN";renderHumanReasoningTrace();renderHumanReflectionCheckpoint();const exec=document.getElementById("handoff-execution");exec.hidden=false;const priority=reviews.filter(r=>r.status==="CHALLENGES"||r.status==="UNCLEAR"||r.status==="NOT_REVIEWED");exec.innerHTML=`<strong>Progressive Execution Queue · HUMAN controlled</strong><p>${priority.length?`${priority.length} paper diprioritaskan dari review saat ini.`:"Tidak ada paper unresolved pada supporting set. Sebelum memperluas retrieval, HUMAN perlu menilai apakah counter-evidence search diperlukan."}</p>${priority.map((r,i)=>`<div class="execution-item"><strong>${i+1}. ${escapeHtml(r.title||r.openalex_id||"Paper")}</strong><br><small>Status review: ${escapeHtml(r.status)}</small></div>`).join("")}<label><input id="allow-expand-retrieval" type="checkbox"> HUMAN mengizinkan perluasan retrieval jika queue ini tidak memenuhi kriteria cukup.</label><button id="prepare-execution" class="primary-action" type="button">Siapkan eksekusi progresif</button><div id="execution-state"></div><small>Tidak ada retrieval tambahan sampai HUMAN mengizinkan. Sistem tidak boleh menganggap queue selesai berarti gap tervalidasi.</small>`;document.getElementById("prepare-execution").onclick=()=>{const allow=document.getElementById("allow-expand-retrieval").checked,state=document.getElementById("execution-state");const payload={opportunity:o.id||o.term,question:q,evidence_contrasts:dimSummary,sufficiency_criterion:suff,priority_papers:priority.map(r=>({openalex_id:r.openalex_id,title:r.title,status:r.status,discovery_role:r.discovery_role,dimensions:r.dimensions,note:r.note})),allow_expand_retrieval:allow,next_action:priority.length?"REVIEW_PRIORITY_PAPERS":allow?"PREPARE_TARGETED_RETRIEVAL":"RETURN_TO_HUMAN",scientific_decision:false,canonical_write:false};sessionStorage.setItem("gfprojclaw-progressive-execution",JSON.stringify(payload));state.innerHTML=`<p><strong>Next action:</strong> ${escapeHtml(payload.next_action)}</p><p>${allow?"Perluasan retrieval diizinkan HUMAN hanya setelah priority queue diperiksa terhadap sufficiency criterion.":"Perluasan retrieval belum diizinkan; kembali ke HUMAN setelah priority queue."}</p>`;document.getElementById("prepare-execution").textContent="EXECUTION DRAFT TERSIAPKAN";renderHumanReasoningTrace();const brief=document.getElementById("handoff-retrieval-brief");brief.hidden=false;const unresolved=priority.map(r=>r.title).filter(Boolean);brief.innerHTML=`<strong>Targeted Retrieval Brief · PRE-EXECUTION</strong><p><strong>Question:</strong> ${escapeHtml(q)}</p><p><strong>Sufficiency criterion:</strong> ${escapeHtml(suff||"Belum ditetapkan HUMAN")}</p>${dimSummary.length?`<div class="callout"><strong>Evidence contrast yang harus dijawab retrieval</strong><br>${dimSummary.map(escapeHtml).join("<br>")}</div>`:""}<p><strong>Known unresolved evidence:</strong> ${unresolved.length?unresolved.map(escapeHtml).join("; "):"Tidak ada pada supporting queue."}</p><p><strong>Retrieval boundary:</strong> ${allow?"HUMAN mengizinkan targeted expansion setelah unresolved queue diperiksa.":"NO EXPANSION AUTHORIZED."}</p><label><strong>Target/context tambahan (opsional, HUMAN)</strong><input id="retrieval-context" type="text" placeholder="mis. Indonesia, municipal governance, citizen outcomes"></label><label><strong>Exact targeted query · HUMAN</strong><input id="retrieval-exact-query" type="text" placeholder="Tulis query yang benar-benar ingin dijalankan; kosong = jangan eksekusi"></label><label><strong>Setelah retrieval, apa yang harus dianggap benar-benar baru?</strong><textarea id="retrieval-novelty-criterion" rows="2" placeholder="HUMAN: contoh konteks baru, metode berbeda, outcome warga, atau mekanisme yang belum terwakili di 19 paper"></textarea></label><button id="save-retrieval-brief" class="secondary-action" type="button">Simpan retrieval brief</button><small>Brief ini belum menjalankan pencarian. Provider/query baru harus tetap traceable dan hasil baru tidak boleh otomatis dianggap relevant/gap.</small>`;document.getElementById("save-retrieval-brief").onclick=()=>{const context=document.getElementById("retrieval-context").value.trim(),exactQuery=document.getElementById("retrieval-exact-query").value.trim(),noveltyCriterion=document.getElementById("retrieval-novelty-criterion").value.trim();sessionStorage.setItem("gfprojclaw-targeted-retrieval-brief",JSON.stringify({opportunity:o.id||o.term,question:q,evidence_contrasts:dimSummary,sufficiency_criterion:suff,unresolved_titles:unresolved,human_context:context,novelty_criterion:noveltyCriterion,baseline_subset:"smart → city → urban · 19 abstracts",expansion_authorized:allow,provider:allow&&exactQuery?"OpenAlex":"NOT_SELECTED",query:allow&&exactQuery?exactQuery:"NOT_EXECUTED",scientific_decision:false,canonical_write:false}));const ready=allow&&exactQuery&&noveltyCriterion;document.getElementById("save-retrieval-brief").textContent=ready?"RETRIEVAL BRIEF SIAP · QUERY + NOVELTY CRITERION HUMAN":"RETRIEVAL BRIEF TERSIMPAN · LENGKAPI OTORISASI, QUERY, DAN KRITERIA BARU";renderHumanReasoningTrace();};};};};};};matrix.querySelectorAll(".evidence-review-actions button").forEach(b=>b.onclick=()=>{const row=b.closest(".evidence-review-row");row.dataset.status=b.dataset.status;row.querySelectorAll("button").forEach(x=>x.classList.toggle("selected",x===b));});};
}
renderExplorerHandoff();

function renderHumanReasoningTrace(){
 const box=document.getElementById("handoff-reasoning-trace");if(!box)return;
 const defs=[
  ["Evidence Review","gfprojclaw-human-evidence-review",x=>`${(x.reviews||[]).length} paper review HUMAN`],
  ["Next Question","gfprojclaw-human-next-investigation",x=>x.question||"draft tersedia"],
  ["Investigation Plan","gfprojclaw-human-investigation-plan",x=>x.sufficiency_criterion?`Kriteria cukup: ${x.sufficiency_criterion}`:"Kriteria cukup belum diisi"],
  ["Execution Queue","gfprojclaw-progressive-execution",x=>`${(x.priority_papers||[]).length} priority paper · ${x.next_action||"state tersedia"}`],
  ["Retrieval Brief","gfprojclaw-targeted-retrieval-brief",x=>x.expansion_authorized?"Targeted expansion diizinkan HUMAN":"Tidak ada expansion authorization"]
 ];
 const rows=[];for(const [label,key,describe] of defs){let x=null;try{x=JSON.parse(sessionStorage.getItem(key)||"null");}catch(_){x=null;}if(x)rows.push({label,key,text:describe(x),safe:x.scientific_decision===false&&x.canonical_write===false});}
 if(!rows.length)return;box.hidden=false;box.innerHTML=`<strong>Bagaimana saya sampai di sini? · HUMAN Reasoning Trace</strong><p>Jejak ini merangkum state sesi browser, bukan kesimpulan ilmiah.</p><ol class="reasoning-trace-list">${rows.map(r=>`<li><strong>${escapeHtml(r.label)}</strong><br>${escapeHtml(r.text)}<br><small>${r.safe?"NON-CANONICAL · scientific_decision=false":"Periksa state sebelum melanjutkan"}</small></li>`).join("")}</ol><button id="refresh-reasoning-trace" class="secondary-action" type="button">Perbarui jejak</button><small> Tool ledger server-side menambahkan fingerprint provenance; tampilan ini hanya membantu HUMAN memahami urutan reasoning pada sesi aktif.</small>`;document.getElementById("refresh-reasoning-trace").onclick=renderHumanReasoningTrace;
}
renderHumanReasoningTrace();

function renderHumanReflectionCheckpoint(){
 const box=document.getElementById("handoff-reflection");if(!box)return;
 let plan=null,review=null;try{plan=JSON.parse(sessionStorage.getItem("gfprojclaw-human-investigation-plan")||"null");review=JSON.parse(sessionStorage.getItem("gfprojclaw-human-evidence-review")||"null");}catch(_){return;}
 if(!plan||!review)return;box.hidden=false;let saved=null;try{saved=JSON.parse(sessionStorage.getItem("gfprojclaw-human-reflection")||"null");}catch(_){}
 box.innerHTML=`<strong>HUMAN Reflection Checkpoint</strong><p>Sebelum loop berikutnya, tuliskan apa yang berubah dalam pemahaman Anda. Sistem tidak mengisi jawaban ini.</p><label><strong>Apa yang sekarang lebih jelas?</strong><textarea id="reflection-clearer" rows="2"></textarea></label><label><strong>Apa yang masih belum pasti?</strong><textarea id="reflection-uncertain" rows="2"></textarea></label><label><strong>Evidence apa yang paling mengubah pemahaman Anda?</strong><textarea id="reflection-evidence" rows="2"></textarea></label><label><strong>Apa yang ingin Anda lakukan berikutnya?</strong><select id="reflection-next"><option value="">Pilih sendiri…</option><option>STOP_AND_REFLECT</option><option>REVIEW_MORE_EVIDENCE</option><option>REVISE_QUESTION</option><option>CONSIDER_TARGETED_RETRIEVAL</option></select></label><button id="save-human-reflection" class="primary-action" type="button">Simpan refleksi HUMAN</button><small>Refleksi adalah catatan reasoning HUMAN; bukan scientific conclusion dan tidak mengubah canonical state.</small>`;
 if(saved){document.getElementById("reflection-clearer").value=saved.clearer||"";document.getElementById("reflection-uncertain").value=saved.uncertain||"";document.getElementById("reflection-evidence").value=saved.evidence_that_changed_understanding||"";document.getElementById("reflection-next").value=saved.intended_next_action||"";}
 document.getElementById("save-human-reflection").onclick=()=>{const payload={clearer:document.getElementById("reflection-clearer").value.trim(),uncertain:document.getElementById("reflection-uncertain").value.trim(),evidence_that_changed_understanding:document.getElementById("reflection-evidence").value.trim(),intended_next_action:document.getElementById("reflection-next").value,based_on_human_review:true,scientific_decision:false,canonical_write:false};sessionStorage.setItem("gfprojclaw-human-reflection",JSON.stringify(payload));document.getElementById("save-human-reflection").textContent="REFLEKSI TERSIMPAN · HUMAN AUTHORED";};
}
renderHumanReflectionCheckpoint();

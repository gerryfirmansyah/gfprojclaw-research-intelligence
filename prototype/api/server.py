    ...
    def do_GET(self):
        ...
        if len(parts) == 4 and parts[0] == "api" and parts[1] == "projects" and parts[3] == "coverage":
            try:
                ...
                self.send_json({"error": str(exc)}, status=400)
            return
        if len(parts) == 4 and parts[0] == "api" and parts[1] == "projects" and parts[3] == "seed-universe":
            try:
                self.send_json(get_project_seed_universe(parts[2]))
            except Exception as exc:
                self.send_json({"error": str(exc)}, status=400)
            return
        if len(parts) == 4 and parts[0] == "api" and parts[1] == "projects" and parts[3] == "exploration":
            try:
                self.send_json(get_project_exploration(parts[2]))
            except Exception as exc:
                self.send_json({"error": str(exc)}, status=400)
            return

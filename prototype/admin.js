...
function setAdminView(name){
 ...
 document.querySelectorAll('[data-admin-panel]').forEach(p=>p.classList.toggle('active',p.dataset.adminPanel===name));
 location.hash=`admin-${name}`;
}
document.querySelectorAll('[data-admin-view]').forEach(b=>b.addEventListener('click',()=>setAdminView(b.dataset.adminView)));
const initialView=(location.hash||'').replace('#admin-',''); if(document.querySelector(`[data-admin-panel="${initialView}"]`)) setAdminView(initialView);
...
async function loadAdmin() {
  ...
  try {
    ...
    pilot.innerHTML=rows.length?rows.map(x=>`<div class="change-row"><div class="change-main"><strong>${esc(x.status)} · ${esc(x.trigger_type)}</strong><small>Run ${esc(x.pilot_run_id)}</small><p>Started: ${esc(x.started_at)}<br>Finished: ${esc(x.finished_at||"RUNNING")}<br>Stage attempts: ${esc(x.stage_attempts)} · Failed attempts: ${esc(x.failed_stage_attempts)}</p><p>Stage history: ${esc((x.stage_history||[]).join(" → ")||"none")}</p></div></div>`).join(""):"<p>No Continuous Pilot run recorded.</p>"; const failures=document.getElementById("admin-failures"); if(failures){const bad=rows.filter(x=>x.status!=="COMPLETED"||Number(x.failed_stage_attempts||0)>0);failures.innerHTML=bad.length?bad.map(x=>`<div class="change-row"><strong>${esc(x.status)} · ${esc(x.trigger_type)}</strong><p>Run ${esc(x.pilot_run_id)} · failed attempts ${esc(x.failed_stage_attempts)}</p></div>`).join(""):"<p>No persisted pilot failure/retry item for this project.</p>";}

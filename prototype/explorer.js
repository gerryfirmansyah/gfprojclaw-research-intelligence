...
function renderTrialPapers(members){
 const shown=(members||[]).slice(0,30);if(trialPaperCount)trialPaperCount.textContent=`${shown.length} / ${(members||[]).length} SHOWN`;
...
function showTrialArea(term){
 ...
 const members=t.members||[],venues=[...new Set(members.map(x=>x.venue).filter(Boolean))].slice(0,5),years=members.map(x=>x.publication_year).filter(Boolean),yr=years.length?`${Math.min(...years)}–${Math.max(...years)}`:"NOT_RECORDED";
 if(trialAdvice)trialAdvice.innerHTML=`<h3>Research Advice · ${esc(term)}</h3><div class="advice-grid"><div class="advice-card"><strong>What we observe</strong>${esc(members.length)} exact title-term members across ${esc(yr)}. Example venues: ${esc(venues.join(", ")||"NOT_RECORDED")}.</div><div class="advice-card"><strong>Why it may matter</strong>This area is visible enough in the bounded landscape to merit inspection if it connects to your research intent. Frequency alone does not establish relevance.</div><div class="advice-card"><strong>What to inspect next</strong>Inspect papers from different years and venues; compare how “${esc(term)}” is used; then inspect abstract/method/context before scientific use.</div><div class="advice-card"><strong>Questions for you</strong>Is “${esc(term)}” central, contextual, or outside your intended scope? Does inspection suggest NARROW, BRANCH, KEEP_SCOPE, or INSPECT_MORE?</div></div><small>Advice is generated from transparent bounded-corpus observations; it is not a relevance, gap, novelty, or evidence decision.</small>`;

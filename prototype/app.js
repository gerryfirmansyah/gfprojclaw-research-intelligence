...
function renderExplorerHandoff(){
 ...
 if(focused){
  ...
  const ev=document.getElementById("handoff-evidence");ev.hidden=false;ev.innerHTML=`<strong>Focused Investigation · assessment HUMAN</strong><div class="copilot-focused-list">${(focused.assessments||[]).map(a=>`<article><strong>${escapeHtml(a.paper||"Paper")}</strong><br><small>${escapeHtml(a.surfaced_as||"")} → <b>${escapeHtml(a.status||"NOT_REVIEWED")}</b></small>${a.note?`<p>Catatan HUMAN: ${escapeHtml(a.note)}</p>`:""}</article>`).join("")}</div>`;

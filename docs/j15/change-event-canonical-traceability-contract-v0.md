...
## HUMAN-readable ChangeEvent presentation — 2026-09-20
Research Copilot now presents Assessment ChangeEvents with a HUMAN-first explanation before technical canonical state. The primary view explains what changed, why it was recorded, the current Assessment meaning, what HUMAN should inspect, and explicitly recorded evidence-trigger membership. Raw Assessment UUIDs and previous/current JSON remain available only under collapsed technical detail. `CANDIDATE` is described as a candidate requiring HUMAN inspection, never as proof of a research gap or scientific acceptance. Empty historical `change_event_evidence` remains UNKNOWN/NOT_RECORDED trigger membership, not evidence absence.
